...
import { mainnetProvider, providerByChain, zksyncProvider, arbitrumProvider, optimismProvider } from "./providers"
...
export function useArbitrumEraBalances({ enabled, wallets }: { enabled: boolean, wallets: PrivateKeyAccount[] }) {
    return useGenericBalanceChecker({ enabled, wallets, provider: arbitrumProvider })
}

export function useOptimismEraBalances({ enabled, wallets }: { enabled: boolean, wallets: PrivateKeyAccount[] }) {
    return useGenericBalanceChecker({ enabled, wallets, provider: optimismProvider as PublicClient })
}
